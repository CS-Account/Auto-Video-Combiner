...
class AutoCombiner:
    ...
    def _collect_video_sets(self, input_folder_path, output_folder_path, input_regex, output_regex, output_template, reverse=False):
        """Collect video sets from input folder using regex."""
        # Get input files.
        input_files = []
        for file in os.listdir(input_folder_path):
            if re.search(input_regex, file):
                input_files.append(os.path.abspath(os.path.join(input_folder_path, file)))
        print("Found {} input files.".format(len(input_files)))

        video_sets = {}
        for input_file in input_files:
            output_filename = self._generate_output_filename(os.path.basename(input_file), output_regex, output_template)
        ...
        parsed_video_sets = {}
        for key in video_sets.keys():
            output_file = os.path.abspath(os.path.join(output_folder_path, self._add_combined_number_to_filename(key, len(video_sets[key]))))
    ...
    def _sort_video_set(self, video_set, input_regex, reverse_order=False):
        ...
        video_set.sort(key=lambda input_file: re.search(input_regex, input_file).group(1), reverse=reverse_order)
    ...
    def _generate_output_filename(self, input_file, output_regex, output_template):
        ...
        if not regex_match:
            ...
            self.errors.append("Input file does not match output regex: {}".format(input_file))
            return None

        # find all non escaped $ signs in output template and replace them with regex group values.
        try:
            output_filename = re.sub(r"(?<!\\)\$(\d+)", lambda match: regex_match.group(int(match.group(1))), output_template)
            output_filename = re.sub(r"\\\$", "$", output_filename)
            return output_filename
        except IndexError:
            print("Output template does not match output regex match (IndexError): {}".format(output_template))
            self.errors.append("Output template does not match output regex match (IndexError): {}".format(output_template))
            return None

    def _add_combined_number_to_filename(self, filename, combined_number):
        output_filename = re.sub(r"(?<!\\)/(\d*)", lambda match: "{:0{}d}".format(combined_number, match.group(1)), filename)
        output_filename = re.sub(r"\\/", "/", output_filename)
        return output_filename
    ...
    def auto_combine_videos(self, input_folder_path, output_folder_path, input_regex, output_regex, output_template, reverse_order=False, dry_run=False, combine_type=COMBINETYPE.SKIP, verbose=False):
        ...
        if not os.path.exists(input_folder_path):
            ...
            self.errors.append("Input folder does not exist: {}".format(input_folder_path))
            return

        # Check if output folder exists.
        if not os.path.exists(output_folder_path):
            print("Output folder does not exist: {}".format(output_folder_path))
            self.errors.append("Output folder does not exist: {}".format(output_folder_path))
            return
        ...
        video_sets = self._collect_video_sets(input_folder_path, output_folder_path, input_regex, output_regex, output_template, reverse_order)
        ...
        for output_file, input_files in video_sets.items():
            ...
            if not file_exists:
                ...
                actions.setdefault("Skipping", []).append((output_file, len(input_files), None))
        ...
        if not dry_run:
            ...
            for action_type, actions_list in actions.items():
                ...
                for output_file, number_of_items, action in actions_list:
                    print("\tProcessing {} ({} sub items)".format(output_file, number_of_items))
                    ...
                    if result:
                        ...
                        print("\n\tError combining video: {} with error: {}\n".format(output_file, result))
        return self.errors


if __name__ == "__main__":
    # Parse command line arguments.
    parser = argparse.ArgumentParser(description="Combine input video files into a single video file using ffmpeg muxing.")
    parser.add_argument("-i", "--input-folder", required=True, help="Folder containing input video files.")
    parser.add_argument("--input-regex", default=r".*([0-9]+)\.[a-zA-Z0-9]*",
                        help="Regular expression for input video files. Grouping around incrementing number is mandatory. (Default: .*([0-9]*)\.[a-zA-Z0-9]*)")
    parser.add_argument("-r", "--reverse_order", action="store_true", help="Reverse order of input files.")
    parser.add_argument("-o", "--output-folder", required=True, help="Folder to save output video files.")
    parser.add_argument("--output-regex", default=r"(.*)(?:[0-9]+)(\.[a-zA-Z0-9]*)",
    ...
    overwrite_combine_group.add_argument("-w", "--overwrite", action="store_true", help="Overwrite existing output file when output file already exists.")
    ...
    combine_option = AutoCombiner.COMBINETYPE.OVERWRITE if args.overwrite else AutoCombiner.COMBINETYPE.COMBINE if args.combine else AutoCombiner.COMBINETYPE.SKIP
